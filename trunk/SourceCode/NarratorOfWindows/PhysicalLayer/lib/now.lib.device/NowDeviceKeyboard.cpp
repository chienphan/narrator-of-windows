...
const WORD NowDeviceKeyboard::VKKEYSCANSHIFTON = 0x01;
const WORD NowDeviceKeyboard::VKKEYSCANCTRLON  = 0x02;
const WORD NowDeviceKeyboard::VKKEYSCANALTON   = 0x04;
const WORD NowDeviceKeyboard::INVALIDKEY       = 0xFFFF;

const BYTE NowDeviceKeyboard::ExtendedVKeys[MaxExtendedVKeys] =
{
	VK_UP, 
	VK_DOWN,
	VK_LEFT,
...
NowDeviceKeyboard::NowDeviceKeyboard()
{
...
//
NowDeviceKeyboard::key_desc_t NowDeviceKeyboard::KeyNames[NowDeviceKeyboard::MaxSendKeysRecs] = 
{
	{_T("ADD"), VK_ADD}, 
	{_T("APPS"), VK_APPS},
	{_T("AT"), '@', true},
...
void NowDeviceKeyboard::KeyboardEvent(BYTE VKey, BYTE ScanCode, LONG Flags)
{
	MSG KeyboardMsg;
...
bool NowDeviceKeyboard::IsVkExtended(BYTE VKey)
{
...
// Generates KEYUP
void NowDeviceKeyboard::SendKeyUp(BYTE VKey)
{
...
void NowDeviceKeyboard::SendKeyDown(BYTE VKey, WORD NumTimes, bool GenUpMsg, bool bDelay)
{
	WORD Cnt = 0;
	BYTE ScanCode = 0;
	bool NumState = false;
...
bool NowDeviceKeyboard::BitSet(BYTE BitTable, UINT BitMask)
{
	return BitTable & BitMask ? true : false;
}

// Sends a single key
void NowDeviceKeyboard::SendKey(WORD MKey, WORD NumTimes, bool GenDownMsg)
{
...
WORD NowDeviceKeyboard::StringToVKey(LPCTSTR KeyString, int &idx)
{
...
void NowDeviceKeyboard::PopUpShiftKeys()
{
	if (!m_bUsingParens)
	{
		if (m_bShiftDown)
...
// Sends a key string
bool NowDeviceKeyboard::SendKeys(LPCTSTR KeysString, bool Wait)
{
	WORD MKey, NumTimes;
...
bool NowDeviceKeyboard::AppActivate(HWND wnd)
{
	if (wnd == NULL)
		return false;
...
BOOL CALLBACK NowDeviceKeyboard::enumwindowsProc(HWND hwnd, LPARAM lParam)
{
...
bool NowDeviceKeyboard::AppActivate(LPCTSTR WindowTitle, LPCTSTR WindowClass)
{
	HWND w;
...
void NowDeviceKeyboard::CarryDelay()
{
...
NowDeviceKeyboard* NowDeviceKeyboard::getInstance()
{
	if (m_instance == NULL)
	{
		m_instance = new NowDeviceKeyboard();
	}
	return m_instance;
}

NowDeviceKeyboard* NowDeviceKeyboard::m_instance = NULL;
