...
"%NET_FRAMEWORK_DIR%\csc.exe" /debug /target:module /define:TRACE /reference:"%NET_REFERENCE_DIR%\UIAutomationClient.dll" /reference:"%NET_REFERENCE_DIR%\UIAutomationTypes.dll" /reference:"%NET_REFERENCE_DIR%\WindowsBase.dll" /out:%OUT_DIR%\now.agent.uiautomation.client.dll %PROJECT_DIR%\*.cs %PROJECT_DIR%\utility\*.cs %PROJECT_DIR%\storage\*.cs %PROJECT_DIR%\logger\*.cs %PROJECT_DIR%\interfaces\*.cs %PROJECT_DIR%\constant\*.cs %PROJECT_DIR%\action\*.cs %PROJECT_DIR%\action\common\*.cs %PROJECT_DIR%\argument\*.cs