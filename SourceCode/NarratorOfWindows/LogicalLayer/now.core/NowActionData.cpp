...
NowActionData::NowActionData(void)
{
	...
	m_mapControlProps = new map<string, map<string, string>*>();
...
NOW_RESULT NowActionData::addControlData( string strWindowName, string strControlName, map<string, string>* strPropsData )
{
	...
	map<string,  map<string, string>*>::iterator itProp = m_mapControlProps->find(strControlName);
	if (itProp == m_mapControlProps->end())
	{
		m_mapControlProps->insert(pair<string, map<string, string>*>(strControlName, strPropsData));
	}
